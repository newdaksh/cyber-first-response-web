...
import { calculateEvidenceCompleteness, demoDescription, officialReportingUrl } from '../lib/services';
  ...
  return (
    <main className={shellClass}>
      ...
      <div id="main-content" tabIndex={-1}>
        ...
        {incident.status === 'INTAKE' && <Intake mode={mode} incident={incident} busy={busy} error={error} onDescription={(description) => updateLocal({ description })} onVoice={simulateVoice} onAnalyze={() => analyze()} onBack={resetDemo} />}
        {incident.status === 'TRIAGE' && <Triage incident={incident} classification={classification} missing={missing} onSubmit={saveClarification} />}
        {incident.status === 'ACTION_REQUIRED' && <ActionPlan incident={incident} onToggle={toggleAction} onContinue={() => advance('EVIDENCE_COLLECTION')} />}
        {incident.status === 'EVIDENCE_COLLECTION' && <EvidenceScreen incident={incident} detected={detected} fileName={fileName} evidenceFields={evidenceFields} busy={busy} error={error} fileRef={fileRef} onChoose={() => fileRef.current?.click()} onFile={(name) => setFileName(name)} onField={(key, value) => setEvidenceFields((current) => ({ ...current, [key]: value }))} onExtract={extractEvidence} onContinue={buildTimeline} />}
        {incident.status === 'TIMELINE_READY' && <TimelineScreen incident={incident} onContinue={() => advance('CASE_READY')} />}
        ...
        {(incident.status === 'COMPLAINT_READY' || incident.status === 'REVIEW') && <ComplaintScreen incident={incident} onChange={(draft) => updateLocal({ complaint: { ...incident.complaint, draft } })} onReview={reviewComplaint} onHandoff={completeHandoff} busy={busy} error={error} />}
        {incident.status === 'HANDOFF' && <Handoff incident={incident} onReset={resetDemo} onTransparency={() => setTransparency(true)} />}
      </div>

      {busy && <div className="busy-overlay" role="status" aria-live="polite"><span className="spinner" /><strong>{busy}</strong><small>Saving your case securely</small></div>}
...
function EvidenceScreen({ incident, detected, fileName, evidenceFields, busy, error, fileRef, onChoose, onFile, onField, onExtract, onContinue }: { incident: Incident; detected: Record<string, string>; fileName: string; evidenceFields: Record<string, string>; busy: string | null; error: string; fileRef: React.RefObject<HTMLInputElement | null>; onChoose: () => void; onFile: (name: string) => void; onField: (key: string, value: string) => void; onExtract: () => void; onContinue: () => void }) {
